            ...
            @elseif($post->post_type == 'live_streaming')
                <div class="row">
                    ...
                    <div class="col-12 text-center pt-5">
                        <a class="live-watch-now mt-3" href="{{ route('live', ['post_id' => $post->post_id]) }}"><i
        ...
        <div class="entry-footer @if (isset($_GET['shared'])) hidden-on-shared-view @endif">
            <div class="footer-share pt-3 d-flex justify-content-around">
                ...
                <span class="entry-react"><a href="javascript:void(0)"
                        onclick="$('#user-comments-{{ $post->post_id }}').toggle();"><i
                            class="fa-solid fa-comment"></i>{{ get_phrase('Comments') }}</a></span>
                <span class="entry-react" data-bs-toggle="modal" data-bs-target="#exampleModal"><a
                        href="javascript:void(0)"
                        onclick="showCustomModal('{{ route('load_modal_content', ['view_path' => 'frontend.main_content.share_post_modal', 'post_id' => $post->post_id]) }}', '{{ get_phrase('Share post') }}');"><i
                            class="fa fa-share"></i>{{ get_phrase('Share') }}</a></span>
                <!-- Post share modal -->
            </div>
