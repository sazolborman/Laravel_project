                    ...
                    <form class="ajaxForm" action="{{route('profile.accept_friend_request')}}" method="post">
	            		...
	            		<input type="hidden" name="user_id" value="{{$user_data->id}}">
                            ...
                            @if($friendAccepted->value('requester') == auth()->user()->id)
                                <button type="button" class="btn btn-primary px-4 no-processing no-uploading" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ get_phrase('Friend request') }}">
                                    {{get_phrase('Requested')}}
                                </button>
                            @else
                                <button type="submit" id="friendRequestConfirmBtn{{$user_data->id}}" class="btn btn-primary px-4 no-processing no-uploading" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ get_phrase('Accept Friend Request') }}">
                                    {{get_phrase('Accept')}}
                                </button>
                            @endif
